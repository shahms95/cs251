...
namespace cs251
{
  ...
  dominos_t::dominos_t()
  {
    ...
    int numRays=128;
     for (int i = 0; i < numRays; i++) {
      float angle = (i / (float)numRays) * 360 * 3.1428/180;
      ...
      bd.position.Set(30.0f,20.0f); // start at blast center
	...
	for(int i=0;i<4;i++){
      b2BodyDef bd;
      bd.position.Set(-28.5f, 9.0f+3.8*i);
    ...
    //The hinged joint 2
 {
	...
	{
	  b2PolygonShape shape;
      shape.SetAsBox(0.2f, 1.0f+0.1f*i);
	  ...
	  bd.position.Set(-25.5f + 1.0f * i, 2.0f-(10-i)*0.1);
	  b2Body* body = m_world->CreateBody(&bd);
	  body->CreateFixture(&fd);
	}
     ...
     for (int i = 0; i < 10; ++i)
	{
      ...
      b2BodyDef bd;
	  bd.type = b2_dynamicBody;
	  bd.position.Set(-25.5f + 1.0f * i, 5.2f);
	  b2Body* body = m_world->CreateBody(&bd);
	  body->CreateFixture(&fd);
	}
      ...
      for (int i = 0; i < 10; ++i)
	{
      ...
      b2BodyDef bd;
	  bd.type = b2_dynamicBody;
	  bd.position.Set(-35.5f + 1.0f * i, 2.0f);
	  b2Body* body = m_world->CreateBody(&bd);
	  body->CreateFixture(&fd);
	}
    }
	...
	b2BodyDef bd;
	bd.type = b2_dynamicBody;
	if(i==0)bd.position.Set(-47.0, 5.0f);
	else
	bd.position.Set(-44.0+1.5*i, 3.5f);
    ...
    //The balls
    {
      ...
      ballbd.position.Set(-18.7f, 9.5f);
      ...
      b2BodyDef ballbd;
      ballbd.type = b2_dynamicBody;
      ballbd.position.Set(-39.1f, 13.5f);
      sbody = m_world->CreateBody(&ballbd);
      sbody->CreateFixture(&ballfd);
    }
